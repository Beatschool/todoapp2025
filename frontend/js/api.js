...
async function displayTasks() {
    try {
        ...
        <div class="d-flex justify-content-between align-items-center">
            ...
            <p class="m-0 text-decoration-line-through opacity-50"><strong>Created on: </strong>${new Date(task.dateCreated).toLocaleDateString()}</p>
        </div>  
        `
        :
        `
        ...
        <div class="d-flex justify-content-between align-items-center">
            ...
            <p class="m-0"><strong>Created on: </strong>${new Date(task.dateCreated).toLocaleDateString()}</p>
        </div>  
        `;
        return li;
    }
    ...
    // Resets the "Create New Tasks" form
    taskForm.reset();
...
async function taskNotCompleted(id) {
       try {
        ...
        const data = await response.json();
        console.log("Task is set to 'not completed'", data);
